...
class Branch(models.Model):
    ...
    cash_journal_ids = fields.Many2many(
        'account.journal', 'branch_cash_journal_rel', 'branch_id', 'journal_id', string='Allowed Cash Journals', domain=[('type', '=', 'cash')])
    bank_journal_ids = fields.Many2many(
        'account.journal', 'branch_bank_journal_rel', 'branch_id', 'journal_id', string='Allowed Bank Journals', domain=[('type', '=', 'bank')])
