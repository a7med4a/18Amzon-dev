...
class MaintenanceExternalJobOrder(models.Model):
    ...
    def action_request_spare_parts(self):
        for rec in self:
            ...
            picking_config_lines = rec.maintenance_request_id.maintenance_team_id.external_maintenance_picking_ids
            print(picking_config_lines)
            if not picking_config_lines:
                raise ValidationError(_('No Picking External configuration for this maintenance team'))
            ...
            for line in rec.component_ids.filtered(lambda x: x.spart_part_request == 'pending'):
                move_vals = {
                    ...
                    'external_job_order_component_id': line.id,
    ...
    #                 'maintenance_external_job_order_id': rec.id,
    #                 'maintenance_request_id': rec.maintenance_request_id.id,
    #                 'external_job_order_component_id': component.id,
...
class StockMove(models.Model):
    ...
    external_job_order_component_id = fields.Many2one(comodel_name='maintenance.external.job.order.component')
