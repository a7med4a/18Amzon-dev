...
class MaintenanceRequestInherit(models.Model):
    ...
    maintenance_type = fields.Selection([('preventive', 'Preventive'), (
        'damage', 'Damage'),  ('accident', 'Accident')], string='Type', default="damage")
    ...
    stage_type = fields.Selection(
        related='stage_id.stage_type', string='Stage Type', readonly=True, tracking=True)
    ...
    vehicle_id = fields.Many2one(
        'fleet.vehicle', string='Vehicle', required=False)
    ...
    vin_sn = fields.Char(string="Chassis Number",
                         related='vehicle_id.vin_sn', store=True)
    usage_type = fields.Selection(
        string="Usage Type", related='vehicle_id.usage_type', store=True)
    route_id = fields.Many2one('branch.route', string="Route")
    request_creation_date = fields.Datetime(
        string='Request Creation Date', required=False, default=fields.Datetime.now(), readonly=True)
    open_date = fields.Datetime(string='Open Date', required=False, copy=False)
    request_close_date = fields.Datetime(
        string='Close Date', required=False, copy=False)
    request_duration = fields.Float(
        string="Duration", compute="_compute_duration")
    maintenance_job_order_ids = fields.One2many(
        comodel_name='maintenance.job.order', inverse_name='maintenance_request_id',)
    maintenance_external_job_order_ids = fields.One2many(
        comodel_name='maintenance.external.job.order', inverse_name='maintenance_request_id')
    move_ids = fields.One2many(
        comodel_name='account.move', inverse_name='maintenance_request_id')
    moves_count = fields.Integer("Moves", compute="_compute_moves_count")
    maintenance_job_order_count = fields.Integer(
        compute="_compute_maintenance_job_order_count")
    maintenance_external_job_order_count = fields.Integer(
        compute="_compute_maintenance_external_job_order_count")
    transfer_ids = fields.One2many(
        comodel_name='stock.picking', inverse_name='maintenance_request_id', string="Transfer")
    transfer_count = fields.Integer(compute="_compute_transfer_count")
    damage_number = fields.Integer()
    accident_number = fields.Integer()
    transfer_old_spare_parts_count = fields.Integer(
        compute="_compute_transfer_count")
    route_branch_domain = fields.Binary(
        string="Route Branch domain", help="Dynamic domain used for Vehicle", compute="_compute_route_branch_domain")
    vehicle_domain = fields.Binary(
        string="Route Branch domain", help="Dynamic domain used for Vehicle", compute="_compute_vehicle_domain")
    allow_maintenance_expense_billing = fields.Boolean(
        related='maintenance_team_id.allow_maintenance_expense_billing', required=False, default=False)
    ...
    def _compute_vehicle_domain(self):
        for maintenance in self:
            domain = [('id', 'not in', self.env['maintenance.request'].search([('stage_type', 'in', ('new', 'under_approval', 'opened'))]).mapped(
                'vehicle_id').ids), ('company_id', '=', self.env.company.id), ('branch_id.branch_type', '=', 'workshop')]
    ...
    def _compute_route_branch_domain(self):
        for maintenance in self:
            ...
            if maintenance.maintenance_team_id:
                domain.append(('destination_branch_id', '=',
                              maintenance.maintenance_team_id.allowed_branch_id.id))
            if maintenance.vehicle_id:
                domain.append(('vehicle_route_ids.fleet_vehicle_id',
    ...
    def _compute_maintenance_job_order_count(self):
        for maintenance in self:
            maintenance.maintenance_job_order_count = len(
                maintenance.maintenance_job_order_ids.ids)

    @api.depends('maintenance_external_job_order_ids')
    def _compute_maintenance_external_job_order_count(self):
        for maintenance in self:
            maintenance.maintenance_external_job_order_count = len(
                maintenance.maintenance_external_job_order_ids.ids)

    @api.depends('transfer_ids')
    def _compute_transfer_count(self):
        for maintenance in self:
            maintenance.transfer_count = len(maintenance.transfer_ids.filtered(
                lambda x: x.is_old_spare_parts == False).ids)
            maintenance.transfer_old_spare_parts_count = len(
                maintenance.transfer_ids.filtered(lambda x: x.is_old_spare_parts == True).ids)
                ...
                if open_shift and close_shift:
                    ...
                    if open_shift.id == close_shift.id:
                        ...
                        rec.request_duration = (
                            open_shift.hour_to - open_time) + (close_time - close_shift.hour_from)

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'maintenance.request.seq')
            if vals['vehicle_id']:
                self.env['fleet.vehicle'].browse(vals['vehicle_id']).write(
    ...
    def action_repair_approval_request(self):
        for rec in self:
            ...
            users = self.env.ref(
                'maintenance_custom.group_approve_for_accident_repair').users
            activity_vals = []
        ...
        for rec in self:
            ...
            if not rec.route_id:
                raise ValidationError(
                    _('Please select a route Branch for this vehicle'))
            if not rec.schedule_date:
                raise ValidationError(
                    _('Please Add a Schedule Date for this request'))
            rec.open_date = fields.Datetime.now()
            current_shift = self.get_current_shift(rec.open_date)
            if not current_shift:
                raise ValidationError(_('No shift found at this time'))
            rec.stage_id = self.env.ref('maintenance.stage_3').id
            rec.vehicle_id.state_id = self.env.ref(
                'fleet_status.fleet_vehicle_state_under_maintenance').id
            if rec.maintenance_type == 'damage' and not rec.damage_id:
                raise ValidationError(
                    _('Please add damage number before opening request'))
            if rec.maintenance_type == 'accident' and not rec.accident_id:
                raise ValidationError(
                    _('Please add Accident number before opening request'))
    ...
    def get_current_shift(self, date):
        for rec in self:
            ...
            current_shift = shifts.filtered(lambda shift: shift.type != 'day_off' and shift.dayofweek ==
                                            day_of_week and shift.hour_from <= open_time_float and shift.hour_to >= open_time_float)
    ...
    def action_close(self):
        ...
        for rec in self:
            ...
            if rec.open_date > rec.request_close_date:
                raise ValidationError(
                    _('Close Date must be greater than Open Date'))
            if rec.maintenance_job_order_ids:
                for job in rec.maintenance_job_order_ids:
                    if job.state not in ('cancelled', 'repaired'):
                        raise ValidationError(
                            _('You must repair or cancel job orders before you close maintenance request'))
            current_shift = self.get_current_shift(rec.request_close_date)
            if not current_shift:
                raise ValidationError(_('No shift found at this time'))
            rec.vehicle_id.state_id = self.env.ref(
                'fleet_status.fleet_vehicle_state_ready_to_transfer_from_workshop').id
            ...
            if rec.maintenance_job_order_ids:
                for job in rec.maintenance_job_order_ids:
                    if job.state != 'cancelled':
                        raise ValidationError(
                            _('You must cancel all job orders before you cancel maintenance request'))
            if rec.stage_type == 'opened':
                rec.vehicle_id.state_id = self.env.ref(
                    'fleet_status.fleet_vehicle_state_ready_to_transfer_from_workshop').id
            else:
                rec.vehicle_id.state_id = self.env.ref(
                    'fleet_status.fleet_vehicle_state_waiting_maintenance').id
    ...
    def action_create_bill(self):
        ...
        view_id = self.env.ref(
            'maintenance_custom.view_maintenance_vendor_bill_form').id
        action['context'] = {
    ...
    def view_expense_bills(self):
        if self.env.user.id in self.maintenance_team_id.notified_accountant_ids.ids:
            ...
            action['domain'] = [('move_type', 'in', [
                                 'in_invoice', 'in_refund']), ('maintenance_request_id', '=', self.id)]
        else:
            action = self.env['ir.actions.actions']._for_xml_id(
                'maintenance_custom.action_maintenance_create_bill')
            action['domain'] = [('move_type', 'in', [
...
class MaintenanceTeamInherit(models.Model):
    ...
    old_spare_parts_operation_type_id = fields.Many2one(
        'stock.picking.type', domain=[('code', '=', 'incoming')])
    allowed_branch_id = fields.Many2one(
        'res.branch', domain=[('branch_type', '=', 'workshop')])
    allowed_branch_ids = fields.Many2many(
        'res.branch', domain=[('branch_type', '=', 'rental')])
    is_quick_maintenance = fields.Boolean(
        string='Quick Maintenance', required=False, default=False)
    allow_maintenance_expense_billing = fields.Boolean(
        required=False, default=False)
    journal_id = fields.Many2one('account.journal', string='Bill Journal', required=True, domain=[
                                 ("type", "=", "purchase")])
                ...
                if rec.vehicle_route_ids:
                    for vehicle in rec.vehicle_route_ids:
                        if vehicle.fleet_vehicle_id.state_id.id == self.env.ref('fleet_status.fleet_vehicle_state_under_maintenance').id:
                            raise ValidationError(
                                _(f'This Vehicle {vehicle.fleet_vehicle_id.name}is already under maintenance'))
