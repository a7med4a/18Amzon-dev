...
class MaintenanceJobOrder(models.Model):
    ...
    def action_repaired(self):
        for rec in self:
            ...
            close_shift = self.get_current_shift(rec.job_order_close_date)
            if self.env['stock.move'].search([('maintenance_job_order_id', '=', rec.id),('picking_id.state', 'not in', ('done', 'cancel'))]):
    ...
    def action_request_spare_parts(self):
        ...
        for rec in self:
            ...
            picking_config_lines = rec.maintenance_request_id.maintenance_team_id.internal_maintenance_picking_ids
            print(picking_config_lines)
            if not picking_config_lines:
                raise ValidationError(_('No Picking configuration for this maintenance team'))
            first_line = picking_config_lines[0]

            picking_vals = {
                'maintenance_request_id': rec.maintenance_request_id.id,
                'maintenance_job_order_id': rec.id,
                'picking_type_id': first_line.operation_id.id,
                'location_id': first_line.location_src_id.id,
                'location_dest_id': first_line.location_dest_id.id,
                'user_ids': first_line.user_ids.ids,
                'number': 1,
                'origin': rec.name,
            ...
            for line in rec.component_ids.filtered(lambda x: x.spart_part_request == 'pending'):
                move_vals = {
                    ...
                    'job_order_component_id': line.id,
    ...
    def action_assign_old_spare_parts(self):
        return {
            ...
            'domain': [('maintenance_job_order_id', '=', self.id)],
            'context': {
                'default_maintenance_job_order_id': self.id,
...
class MaintenanceJobOrderComponent(models.Model):
    ...
    done_qty = fields.Float(string="Done Quantity",readonly=True)
    ...
    @api.depends('product_category_id','maintenance_job_order_id','maintenance_job_order_id.vehicle_id')
...
class StockPicking(models.Model):
    ...
    show_next_pickings_maintenance = fields.Boolean(default=False)
    number = fields.Integer(string='Number',required=False)
    user_ids = fields.Many2many('res.users', 'res_stock_picking_users_rel', 'bid', 'user_id', string='Allowed Picking Users')
    ...
    def button_validate(self):
        ...
        for rec in self:
            team = rec.maintenance_request_id.maintenance_team_id
            is_internal = bool(rec.maintenance_job_order_id)
            is_external = bool(rec.maintenance_external_job_order_id)

            if is_internal or is_external:
                picking_config_lines = (
                    team.internal_maintenance_picking_ids if is_internal
                    else team.external_maintenance_picking_ids
                ).filtered(lambda x: x.sequence == rec.number + 1)

                if picking_config_lines:
                    next_picking = rec.copy()
                    for line, next_line in zip(rec.move_ids_without_package, next_picking.move_ids_without_package):
                        next_line.product_uom_qty = line.quantity

                    config = picking_config_lines
                    next_picking.write({
                        'number': rec.number + 1,
                        'picking_type_id': config.operation_id.id,
                        'location_id': config.location_src_id.id,
                        'location_dest_id': config.location_dest_id.id,
                        'user_ids': config.user_ids.ids,
                    ...
                    for line in rec.move_ids_without_package:
                        component = (
                            line.job_order_component_id if is_internal
                            else line.external_job_order_component_id
                        )
                        component.done_qty = line.quantity
                        component.picking_status = 'done'
    ...
    def action_next_transfer(self):
        ...
        picking_domain = []
        if self.maintenance_job_order_id:
            picking_domain = [('maintenance_job_order_id', '=', self.maintenance_job_order_id.id),('number', '=', self.number + 1)]
        elif self.maintenance_external_job_order_id:
            picking_domain = [('maintenance_external_job_order_id', '=', self.maintenance_external_job_order_id.id),('number', '=', self.number + 1)]
        if picking_domain:
            next_transfer = self.env['stock.picking'].search(picking_domain)
            if next_transfer:
                return {
                    "type": "ir.actions.act_window",
                    "res_model": "stock.picking",
                    "views": [[False, "form"]],
                    "res_id": next_transfer[-1].id
                }
