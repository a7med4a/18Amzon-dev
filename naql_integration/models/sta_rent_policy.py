...
class NaqlRentPolicy(models.Model):
    _name = 'sta.rent.policy'
    _description = 'Naql Rent Policy Integration'
    _order = 'name'

    name = fields.Char(
        string='Policy Name',
        required=True,
        help='Name of the rental policy in Naql system'
    )
    
    sta_policy_id = fields.Integer(
        string='Naql Policy ID',
        required=True,
        unique=True,
        help='Policy ID in Naql system'
    )
    ...
    def sync_policies_from_sta(self):
        """Synchronize rental policies from Naql system"""
        ...
        try:
            ...
            if result.get('success'):
                ...
                _logger.info(f"Synchronized {synced_count} rental policies from Naql")
                return {
                    'success': True,
    ...
    def action_sync_policies(self):
        """Action to sync policies from Naql"""
