...
class STACancelContractWizard(models.TransientModel):
    _name = 'sta.cancel.contract.wizard'
    _description = 'Cancel STA Contract'

    sta_contract_id = fields.Many2one(
        'sta.contract',
        string='STA Contract',
        required=True,
        help='STA contract to cancel'
    )
    ...
    def action_cancel_contract(self):
        """Cancel the contract in STA system"""
        self.ensure_one()
        ...
        try:
            ...
            if result.get('success'):
                ...
                return {
                    ...
                    'params': {
                        'title': _('Success'),
                        'message': _('Contract cancelled successfully in STA system.'),
                        'type': 'success',
                    }
                }
            else:
            ...
            _logger.error(f"Error cancelling STA contract: {str(e)}")
            self.sta_contract_id.write({
                'sta_status': 'error',
                'error_message': str(e),
            })
