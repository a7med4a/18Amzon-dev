...
class STACloseContractWizard(models.TransientModel):
    _name = 'sta.close.contract.wizard'
    _description = 'Close STA Contract'

    sta_contract_id = fields.Many2one(
        'sta.contract',
        string='STA Contract',
        required=True,
        help='STA contract to close'
    )
    ...
    def action_close_contract(self):
        """Close the contract in STA system"""
        self.ensure_one()
        ...
        try:
            ...
            if result.get('success'):
                ...
                return {
                    ...
                    'params': {
                        'title': _('Success'),
                        'message': _('Contract closed successfully in STA system.'),
                        'type': 'success',
                    }
                }
            else:
            ...
            _logger.error(f"Error closing STA contract: {str(e)}")
            self.sta_contract_id.write({
                'sta_status': 'error',
                'error_message': str(e),
            })
