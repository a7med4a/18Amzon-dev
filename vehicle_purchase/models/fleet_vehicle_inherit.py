...
class InheritFleetVehicle(models.Model):
    ...
    vehicle_purchase_order_line_ids = fields.Many2many(
        'vehicle.purchase.order.line', readonly=True)

    def write(self, values):
        res = super(InheritFleetVehicle, self).write(values)
        for rec in self:
            align_items = self.env['account.move.line'].search(
                [('vehicle_id', '=', self.id)])
            align_assets = self.env['account.asset'].search(
                [('vehicle_id', '=', self.id)])
    ...
    def view_related_purchase_order(self):
        self.ensure_one()
        return {
            'name': 'Purchase Order',
